...
const { cheerio, request, url, _ } = require('../config/commonModules');
const util = url;
const { req } = require('../config/utils');
...
async function getDataByPage(url) {
	...
	// });
	return getItems(await request(options));
}
...
function getPagination(url) {
	...
	return req(options).then(function (body) {
		let $ = cheerio.load(body.body);
		let pages = $('.zm-invite-pager span')
			.eq(-2)
			.text();
		let currentPage = $('.zm-invite-pager span.zg-gray-normal')
			.eq(-1)
			.text();
		return { pages: parseInt(pages), current: parseInt(currentPage) };
	});
}
...
function getAllPageData(url) {
	...
	return getPagination(url).then(function (paginations) {
		...
		return new Promise((resolve) => {
			_.forEach(pages, async function (page) {
				let pageUrl = realUrl + '?page=' + page;
				let items = await getDataByPage(pageUrl);
				allItems.push(items);
				if (allItems.length == pages.length) {
					resolve(allItems);
				}
			});
		})
